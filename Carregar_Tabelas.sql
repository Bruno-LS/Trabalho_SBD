...
AS
BEGIN
	INSERT INTO Movimentação_Estoque (ID_Produto, ID_Pedido, Estoque) SELECT IT.ID_Produto, IT.ID_Pedido, PR.Estoque-IT.Quant AS Estoque FROM Produto PR INNER JOIN Itens_Pedidos IT ON (IT.ID_Produto=PR.ID_Produto) LEFT JOIN Pedido PE  ON (IT.ID_Pedido=PE.ID_Pedido) ORDER BY PE.Valor_Total DESC
	UPDATE Produto SET Estoque = MO.Estoque FROM Movimentação_Estoque MO WHERE Produto.ID_Produto = MO.ID_Produto;
END
GO
